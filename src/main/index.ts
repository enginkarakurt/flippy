...
const db = new Database(join(app.getPath('userData'), 'flippy.db'))
...
async function executeDbQuery(query: string): Promise<Database.RunResult> {
  try {
...
async function getFlips(): Promise<unknown[]> {
  const flips = db.prepare('SELECT * FROM flips ORDER BY created_at DESC').all()
  return flips
}
